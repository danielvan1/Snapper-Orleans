    ...
    public class DeterministicTransactionExecutor : IDeterministicTransactionExecutor
    {
        ...
        public async Task WaitForTurn(TransactionContext context)
        {
            ...
            if (context.RegionalBid != -1)
            {
                ...
                // this.logger.LogError("RegionalBid: {bid} -> LocalBid: {lbid} ----- RegionalLocalTid: {tid} -> LocalTid: {ltid}",
                //                      context.RegionalBid, this.regionalBidToLocalBid[context.RegionalBid], context.RegionalTid, this.regionalBidToRegionalTidToLocalTidPerBatch[context.RegionalBid][context.RegionalTid]);
        ...
        {
            this.logger.LogError("Received LocalSubBatch: {batch}",
