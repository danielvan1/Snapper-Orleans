...
{
    /// <summary>
    /// Grain to configure the localcoordinators. This is assumed to be activated in a single silo per region.
    /// </summary>
    ...
    public class LocalConfigurationGrain : Grain, ILocalConfigGrain
    {
        ...
        public async Task InitializeLocalCoordinators(string currentRegion)
        {
            ...
            // regionAndServerKey should be EU-EU-1
            // which indicate: <deployed region>-<home region>-<server id>
            foreach (string siloId in siloIds)
            {
                this.logger.LogInformation("Activating localcoordinators in silo: {siloId}", siloId);
